...
@property (nonatomic,strong) NSString *RevealThePassengersList;

@property (nonatomic,strong) NSString *_TrueOrFalse;
@property (nonatomic,strong) NSMutableArray *_TrueOrFalse2;
@property (nonatomic ,strong) NSMutableArray *reviews ;
@property (nonatomic ,strong) NSMutableArray *markers ;
@property (nonatomic ,strong) NSArray *passengers ;
@property (nonatomic ,strong) NSArray *PermitsShowButton ;
...
@implementation RideDetailsVCInvitePassengers
...
LNationality.text = GET_STRING(@"nationality");
    ...
    else if (self.createdRide){//RideDetailsVCInvitePassengers
        ...
        [[MasterDataManager sharedMasterDataManager] getReviewList:routeDetails.AccountId.stringValue andRoute:routeDetails.ID.stringValue withSuccess:^(NSMutableArray *array) {
            ...
            if (blockSelf.createdRide || blockSelf.driverDetails) {
                [[MasterDataManager sharedMasterDataManager] getPassengersByRouteId:blockSelf.routeDetails.ID.stringValue withSuccess:^(NSMutableArray *array) {
                    ...
                    else /* THE SAME CODE FOR THE NOTEMPTYLIST*/
                    {
                        thirdButton.alpha = 0;
                        NSLog(@"DAM UUUUUU ");
                        NSMutableArray *brokenCars = [[NSMutableArray alloc] init];
                        for (Passenger *passenger in array) {
                            NSString *PassengerNameValue = [NSString stringWithFormat:@"%@",passenger.PassengerStatus];
                            NSString *DriverNameValue = [NSString stringWithFormat:@"%@",passenger.RequestStatus];
                            
                            if ([PassengerNameValue containsString:@"1"] && [DriverNameValue containsString:@"1"]){
                                NSLog(@"testPassengerName contain Null");
                                __TrueOrFalse = @"True";
                                NSLog(@"%@",__TrueOrFalse);
                            }else {
                                NSLog(@"__TrueOrFalse doesn't contain True Result");
                            }
                            
                            
                            if([__TrueOrFalse isEqual:@"True"] && [__TrueOrFalse isEqual:@"True"]){
                                _RevealThePassengersList = @"Yes";
                                blockSelf.passengers = brokenCars;
                                NSLog(@"Entered the For>if request status True");
                            }
                            
                            NSLog(@"RequestStatus: (%@)",passenger.RequestStatus);
                            NSLog(@"passengerStatus: (%@)",passenger.PassengerStatus);
                            
                            
                            if([__TrueOrFalse isEqual:@"True"]){
                                NSLog(@"_trueOrFalse is True");
                                //
                                [brokenCars addObject:passenger ];
                                thirdButton.alpha = 1;
                                [blockPassengersList reloadData];
                                [KVNProgress dismiss];
                                if(blockSelf.driverDetails){
                                    NSString * applicationUserID = [[MobAccountManager sharedMobAccountManager] applicationUserID];
                                    NSLog(@"That is the so called application user: %@",applicationUserID);
                                    if (applicationUserID.length > 0) {
                                        if([passenger.AccountId.stringValue isEqualToString:applicationUserID]){
                                            if([passenger.RequestStatus isEqual:@"True"] && [passenger.PassengerStatus isEqual:@"True"]){
                                                blockSelf.alreadyJoined = YES;
                                                NSLog(@"RequestStatus : %@:",passenger.RequestStatus);
                                                NSLog(@"RequestStatus : %@:",passenger.PassengerStatus);
                                                break;
                                            }else {
                                                joinRideBtn.alpha = 0;
                                                CGSize contentSize = contentView.contentSize ;
...
- (IBAction)MatchedSearchResulst:(id)sender {
    NSInteger NumberOfSeats = ([self.routeDetails.NoOfSeats integerValue]);
